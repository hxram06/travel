...
const TOKYO = (() => {
  ...
  // 코인락커 안내(일반 위치). 정확한 층·빈자리·요금은 현장/앱에서 확인.
  const lockerInfo = {
    narita: '각 터미널 도착·출발층에 코인락커와 수하물 보관 카운터',
    keisei: '게이세이우에노역 개찰 안팎에 코인락커(스카이라이너 도착 직후 이용)',
    ueno: 'JR 우에노역 중앙·시노바즈·공원 개찰구 주변에 대·중·소 다수. 아메요코 방면은 시노바즈구치 쪽이 편해요',
    tamachi: 'JR 다마치역 개찰 안과 시바우라구치 주변에 코인락커',
    tokyoJR: '도쿄역 마루노우치·야에스 지하와 각 개찰 주변에 대규모(야에스 지하가 특히 많음)',
    maihama: 'JR 마이하마역 개찰 밖 디즈니 방면에 코인락커',
    shibuya: '시부야역 하치코 개찰·지하·JR 각 개찰 주변. 스크램블스퀘어·히카리에에도 있음',
    harajuku: 'JR 하라주쿠역 신역사 개찰 주변(다케시타구치·오모테산도구치)',
    omotesandoStn: '오모테산도역 개찰 주변 코인락커(도쿄메트로)',
    shinjuku: '신주쿠역 동·서·남 각 개찰과 지하에 매우 많음(잘 참 → 여러 곳 확인)',
    shinjukuM: '도쿄메트로 신주쿠역 개찰 주변 코인락커',
    ginza: '긴자역 지하 개찰 주변 코인락커(도쿄메트로)',
    asakusa: '아사쿠사역(긴자선·도에이) 개찰 주변, 가미나리몬 관광안내소 인근에도',
    yoyogi: 'JR 요요기역 개찰 주변에 소규모 코인락커',
    tokyoKeiyo: '도쿄역 게이요선 통로·야에스 지하에 코인락커',
  };
  for (const [id, note] of Object.entries(lockerInfo)) if (places[id]) places[id].locker = note;
  const lines = {
  ...
  const days = [
    { id: 'd1', date: '1.25', weekday: '월', title: '우에노에서 시작하는 첫날', subtitle: '공항에서 바로 우에노로. 짐을 맡기고 가볍게.', steps: [
      ...
      journey('ueno-walk', '우에노를 가볍게 둘러보기', 'ameyoko', '저녁 · 자유롭게', ['ueno-ameyoko'], '아메요코와 야마시로야 등 캐릭터 매장을 둘러봐요. 매장별 마감이 다르니 굿즈 쇼핑부터.', { photo: 'ueno', highlights: ['아메요코 거리', '야마시로야 캐릭터 굿즈', '드럭스토어·간식'], without: 'asakusa', spots: [{ name: '야마시로야', query: 'ヤマシロヤ 上野' }, { name: '돈키호테 우에노', query: 'ドン・キホーテ 上野' }, { name: '우에노 마루이', query: '上野マルイ' }, { name: 'ABAB 우에노', query: 'ABAB 上野' }] }),
    ...
    { id: 'd3', date: '1.27', weekday: '수', title: '좋아하는 것들과, 노을', subtitle: '하라주쿠 · 시부야 · 시부야 스카이', steps: [
      ...
      journey('harajuku-out', '다마치 → 하라주쿠', 'harajuku', '10:30 전후 도착 목표', ['tamachi-enter', 'tamachi-harajuku'], '야마노테선 외선 직통으로 하라주쿠까지(환승 없음). 산리오·다케시타 쪽부터 시작해요.', { spots: [{ place: 'harakado' }, { name: '@cosme TOKYO', query: '@cosme TOKYO 原宿' }, { name: '라포레 하라주쿠', query: 'ラフォーレ原宿' }, { name: 'WITH HARAJUKU', query: 'WITH HARAJUKU 原宿' }, { name: '키디랜드 하라주쿠', query: 'キデイランド原宿' }, { name: '도큐플라자 오모카도', query: '東急プラザ表参道原宿' }] }),
      journey('harajuku', '산리오 · 다마고치 · 오모테산도, 하라주쿠 쇼핑', 'omotesandoStn', '오전~오후 · 2:00~2:30 마무리', ['harajuku-omotesando', 'omotesando-stn'], '산리오 하라주쿠·다케시타도리에서 시작해 다마고치 팩토리를 지나 오모테산도 사거리와 그 아래 오모테산도역까지 상권을 한 줄기로 둘러봐요. 지도에 쇼핑 거점이 함께 표시돼요. 2시~2시 반쯤 마무리하고 시부야로 넘어가요.', { photo: 'omotesando', highlights: ['산리오 하라주쿠 · 다케시타도리', '다마고치 팩토리 · 하라카도 3층 (현재 11:00~21:00)', '오모테산도 사거리 · 키디랜드 · 애플 오모테산도', '도큐플라자 · 오모테산도 힐스 · GYRE · 스파이럴'], source: 'https://tamagotchi-factory.jp/', spots: [{ place: 'harajuku' }, { place: 'harakado' }, { name: '다케시타도리', query: '竹下通り' }, { name: '오모테산도 힐스', query: '表参道ヒルズ' }, { name: '애플 오모테산도', query: 'Apple 表参道' }, { name: 'niko and… 오모테산도', query: 'niko and TOKYO 表参道' }, { name: 'GYRE 오모테산도', query: 'GYRE 表参道' }, { name: '스파이럴 아오야마', query: 'スパイラル 青山' }, { name: '오모테산도역', query: '表参道駅' } ] }),
      step('harajuku-lunch', 'meal', '하라주쿠에서 점심', 'omotesando', '점심', '오모테산도·하라주쿠 골목에서 점심을 먹고 시부야로 이동해요.', { meal: 'harajuku', spots: [{ place: 'harajuku' }] }),
      journey('shibuya-in', '하라주쿠 → 시부야', 'scramble', '오후 2시대 이동', ['omotesandoStn-shibuya', 'shibuya-scramble'], '오모테산도역에서 긴자선으로 시부야 한 정거장(하라주쿠에서 바로 간다면 JR 야마노테 시부야행 한 정거장도 좋아요). 하치코 쪽에서 시부야 쇼핑을 시작해요.'),
      journey('donki', '시부야 쇼핑 · 메가 돈키', 'donki', '오후 · 자유롭게', ['scramble-donki'], '메가 돈키호테 시부야 본점을 중심으로 센터가이·백화점을 둘러봐요.', { photo: 'shibuya', photoPlace: 'scramble', highlights: ['MEGA 돈키호테 시부야 본점', '시부야 파르코 · 로프트', '패션·뷰티·드럭스토어'], spots: [{ name: '시부야 파르코(포켓몬·닌텐도)', query: '渋谷PARCO' }, { name: '시부야 로프트', query: '渋谷ロフト' }, { name: '시부야 109', query: 'SHIBUYA109' }, { name: '시부야 히카리에', query: '渋谷ヒカリエ' }, { name: '만다라케 시부야', query: 'まんだらけ 渋谷' }, { name: '타워레코드 시부야', query: 'タワーレコード 渋谷' }, { name: '도큐플라자 시부야', query: '東急プラザ渋谷' }, { name: '시부야 마루이', query: '渋谷マルイ' }] }),
    ...
    { id: 'd4', date: '1.28', weekday: '목', title: '마지막 쇼핑, 도쿄의 밤', subtitle: '신주쿠 · 긴자 · 마루노우치', steps: [
      ...
      step('shinjuku', 'visit', '신주쿠에서 원하는 것 찾기', 'shinjukuShop', '오전 · 자유롭게', '캐릭터 굿즈, 옷, 화장품. 필요한 매장부터 골라 둘러보고 점심을 먹어요.', { photo: 'shinjuku', highlights: ['산리오·캐릭터 굿즈', '백화점·뷰티', '필요한 매장 중심으로'], spots: [{ name: '돈키호테 신주쿠 동남쪽', query: 'ドン・キホーテ 新宿東南口店' }, { name: '신주쿠 마루이 아넥스', query: '新宿マルイ アネックス' }, { name: '뉴우먼 신주쿠', query: 'NEWoMan 新宿' }, { name: '이세탄 신주쿠', query: '伊勢丹 新宿店' }, { name: '신주쿠 다카시마야', query: '新宿高島屋' }, { name: '루미네 신주쿠', query: 'ルミネ新宿' }, { name: '디즈니 플래그십 도쿄', query: 'Disney Flagship Tokyo 新宿' }, { name: '요도바시 신주쿠', query: 'ヨドバシカメラ 新宿西口本店' }, { name: '핸즈 신주쿠', query: 'ハンズ 新宿' }] }),
      ...
      journey('ginza', '긴자에서 마지막 쇼핑', 'ginzaShop', '오후 · 자유롭게', ['sanrio-ginza'], '산리오 NISHIGINZA부터 백화점·뷰티·선물까지. 매장 마감 전에 구매를 정리해요.', { photo: 'ginza', highlights: ['산리오 NISHIGINZA · 2층', '백화점·드럭스토어', '선물과 면세 쇼핑'], spots: [{ place: 'sanrio' }, { name: '긴자 로프트', query: '銀座ロフト' }, { name: '돈키호테 긴자', query: 'ドン・キホーテ 銀座本館' }, { name: '긴자 미쓰코시', query: '銀座三越' }, { name: 'GINZA SIX', query: 'GINZA SIX' }, { name: '마쓰야 긴자', query: '松屋銀座' }, { name: '이토야 긴자', query: '銀座 伊東屋' }, { name: '무인양품 긴자', query: '無印良品 銀座' }, { name: '유니클로 긴자', query: 'ユニクロ 銀座店' }] }),
